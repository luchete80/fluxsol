/*
 * vtkImRenderWindowInteractor - class to enable VTK to render to and interact
 * with a FLTK window.
 *
 ...
 *
 * $Id: vtkImRenderWindowInteractor.cxx,v 1.25 2006/03/13 08:50:53 cpbotha Exp $
 */
...
//---------------------------------------------------------------------------
vtkImRenderWindowInteractor::vtkImRenderWindowInteractor( int x, int y, int w, int h, const char *l )
// :
...
//---------------------------------------------------------------------------
vtkImRenderWindowInteractor * vtkImRenderWindowInteractor::New()
{
    // we don't make use of the objectfactory, because we're not registered
    return new vtkImRenderWindowInteractor;
}

//---------------------------------------------------------------------------
void vtkImRenderWindowInteractor::Initialize()
{
    // if don't have render window then we can't do anything yet
    if (!RenderWindow)
    {
	vtkErrorMacro(<< "vtkImRenderWindowInteractor::Initialize has no render window");
	return;
    }
...
//---------------------------------------------------------------------------
void vtkImRenderWindowInteractor::Enable()
{
...
//---------------------------------------------------------------------------
void vtkImRenderWindowInteractor::Disable()
{
...
//---------------------------------------------------------------------------
void vtkImRenderWindowInteractor::Start()
{
    // the interactor cannot control the event loop
    vtkErrorMacro(<<"vtkImRenderWindowInteractor::Start() interactor cannot control event loop.");
}
//---------------------------------------------------------------------------
void vtkImRenderWindowInteractor::SetRenderWindow(vtkRenderWindow *aren)
{
   vtkRenderWindowInteractor::SetRenderWindow(aren);
   // if a vtkImRenderWindowInteractor has been shown already, and one
...
void vtkImRenderWindowInteractor::UpdateSize(int W, int H)
{
...
//---------------------------------------------------------------------------
int vtkImRenderWindowInteractor::CreateTimer(int timertype)
{
...
//---------------------------------------------------------------------------
int vtkImRenderWindowInteractor::DestroyTimer()
{
    // do nothing
    return 1;
}

void vtkImRenderWindowInteractor::OnTimer(void)
{
    if (!Enabled)
      return;
...
//---------------------------------------------------------------------------
void vtkImRenderWindowInteractor::TerminateApp()
{
    ;
}
...
// FLTK event handlers
//---------------------------------------------------------------------------
void vtkImRenderWindowInteractor::flush(void)
{
...
//---------------------------------------------------------------------------
void vtkImRenderWindowInteractor::draw(void){
    if (RenderWindow!=NULL)
    {
...
//---------------------------------------------------------------------------
void vtkImRenderWindowInteractor::resize( int x, int y, int w, int h ) {
...
int vtkImRenderWindowInteractor::handle( int event ) {
//    if( !Enabled ) return 0;
//
//#if (VTK_MAJOR_VERSION >= 4)
//    // setup for new style
...
const char *vtkImRenderWindowInteractor_rcsid(void)
{
    return rcsid;
}
