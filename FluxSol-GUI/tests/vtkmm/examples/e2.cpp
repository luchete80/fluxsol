...
const float MW::_sc_kx, MW::_sc_ky, MW::_sc_kz;
const float MW::_sc_px, MW::_sc_py, MW::_sc_pz;
const float MW::_sc_min, MW::_sc_max;
...
float Function::EvaluateFunction(float x[3])
{
	float ret = 0;
	for(unsigned i=0; i != 3; ++i)
...
void Function::EvaluateGradient(float x[3], float g[3])
{
	for(unsigned i=0; i !=3; ++i)
...
{
	_a_k[0] = static_cast<float>(_spinKx.get_value());
	_a_k[1] = static_cast<float>(_spinKy.get_value());
	_a_k[2] = static_cast<float>(_spinKz.get_value());
	
	_a_p[0] = static_cast<float>(_spinPx.get_value());
	_a_p[1] = static_cast<float>(_spinPy.get_value());
	_a_p[2] = static_cast<float>(_spinPz.get_value());
	
	_a_range[0] = static_cast<float>(_spinMin.get_value());
	_a_range[1] = static_cast<float>(_spinMax.get_value());
