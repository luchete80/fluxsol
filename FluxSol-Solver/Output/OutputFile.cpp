...
//Output File functions w/o template arguments
...
template <typename T>
void OutputFile::WriteField(const Vertex_Fv_Field<T> &field)
{
    int rank=int(pTraits<T>::rank);
	...
	//If scalar
	for (int n=0;n<this->grid.Num_Verts();n++)
    {
        for (int d=0;d<comp;d++)
            file << field.Val(n).Comp()[d] <<" ";
        file <<endl;
