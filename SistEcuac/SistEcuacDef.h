...
*************************************************************************/
#ifndef _SISTECUACDEF_H
#define _SISTECUACDEF_H
...
template <typename T>
int FluxSol::Eqn<T>::MinNeigbourId()
{
	int min=1e10;
	for (int i=0;i<this->neighbour_id.size();i++)
	{
...
template <typename T>
void FluxSol::EqnSystem<T>::Insert (const FluxSol::Eqn <T> &ec)
{
	eqn.push_back(ec);
}
