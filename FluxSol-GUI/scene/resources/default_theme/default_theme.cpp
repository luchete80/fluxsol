...
static Ref<Font> make_font2(int p_height,int p_ascent, int p_charcount, const int *p_char_rects,int p_kerning_count,const int *p_kernings,int p_w, int p_h, const unsigned char *p_img) {
	...
	print_line("creating tex");
	Ref<ImageTexture> tex = memnew( ImageTexture ); //TOMODIFY, LUCIANO
    print_line("created tex");
	tex->create_from_image(image); //TOMODIFY, LUCIANO

	font->add_texture( tex ); //TOMODIFY, LUCIANO
...
void make_default_theme() {
	...
	// Panel
    print_line("Panel");
	t->set_stylebox("panel","Panel", make_stylebox( panel_bg_png,0,0,0,0) ); //LUCIANO
