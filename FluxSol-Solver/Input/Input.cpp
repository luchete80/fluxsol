...
//            bf.Add_PatchField(udo);
            ...
            // TO MODIFY; ASSIGNING GRID FROM READ_CGNS DOES NOT WORK
            udo->_Patch().AddGrid(*GridPtr);
            //cout << "Grid Num Cells: "<< udo->_Patch().Grid().Num_Cells()<<endl;

//            for (int p=0;p<GridPtr->vBoundary().vPatch(meshp).Num_Faces();p++)
//            {
//                int idface=GridPtr->vBoundary().vPatch(meshp).Id_Face(p);
//                Vec3D v=GridPtr->Face(idface).Center();
//                cout << "Value: " <<v.outstr()<<endl;
//                cout << "Id Face "<< idface<<endl;
//            }
