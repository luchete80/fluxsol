...
//     #define DIAG_S 0.0   //Softening
      ...
      this->ierr  = MPI_Comm_rank(this->comm, &rank);CHKERRQ(this->ierr);     //ATENTION!!!: Is like mype in example 54
      this->ierr  = MPI_Comm_size(this->comm, &npe);CHKERRQ(this->ierr);
      ...
      // //PetscErrorCode  MatCreateAIJ(MPI_this->comm this->comm,PetscInt m,PetscInt n,PetscInt M,PetscInt N,
...
//    MatSetUp(this->A);
   ...
   bs   = 1;
   /* Generate vectors */
   this->ierr = VecCreate(this->comm,&this->x);CHKERRQ(this->ierr);
...
// //
    ...
    // cout << "Coordinates created. "<<endl;
     ...
     this->ierr = KSPSetTolerances(this->ksp,1.e-2,1.e-2,PETSC_DEFAULT,PETSC_DEFAULT);CHKERRQ(this->ierr);
   ...
   if (!PETSC_TRUE)
     {
     PetscViewer viewer;
     this->ierr = PetscViewerASCIIOpen(this->comm, "Amat.m", &viewer);CHKERRQ(this->ierr);
  ...
  // ierr = KSPGetIterationNumber(ksp,&its);CHKERRQ(ierr);
    ...
    cout <<"Solver Results "<<endl;
	 for (int e=0;e<TEqn.Num_Eqn();e++)
	 {
         ...
         for (int dim=0;dim<numberofcomp;dim++)
         {
             //r.push_back((double)this->x[numberofcomp*e+dim]);
             r.push_back(this->X(numberofcomp*e+dim));
             //cout << "xi= "<<numberofcomp*e+dim<<", ";
             //cout <<U.Cmp[numberofcomp*e+dim+1]<<" ";
         }
        TEqn.Eqn(e).X()=r;
    ...
    ittime_spent = (double)(clock() - ittime_end) / CLOCKS_PER_SEC;
     ...
     cout << "PETSC Alloc Solution Time: "<<ittime_spent<<endl;
     ittime_end = clock();
   ...
   if (!PETSC_TRUE) {
     ...
     PetscViewer viewer;
     Vec         res;
     this->ierr = PetscViewerASCIIOpen(this->comm, "rhs.m", &viewer);CHKERRQ(this->ierr);
     this->ierr = PetscViewerSetFormat(viewer, PETSC_VIEWER_ASCII_MATLAB);CHKERRQ(this->ierr);
     this->ierr = VecView(this->b,viewer);CHKERRQ(this->ierr);
     this->ierr = PetscViewerDestroy(&viewer);
     this->ierr = VecNorm(this->b, NORM_2, &norm2);CHKERRQ(this->ierr);

     this->ierr = PetscViewerASCIIOpen(this->comm, "solution.m", &viewer);CHKERRQ(this->ierr);
     this->ierr = PetscViewerSetFormat(viewer, PETSC_VIEWER_ASCII_MATLAB);CHKERRQ(this->ierr);
     ...
     this->ierr = PetscViewerASCIIOpen(this->comm, "residual.m", &viewer);CHKERRQ(this->ierr);
     this->ierr = PetscViewerSetFormat(viewer, PETSC_VIEWER_ASCII_MATLAB);CHKERRQ(this->ierr);
     this->ierr = VecView(this->b,viewer);CHKERRQ(this->ierr);
     this->ierr = PetscViewerDestroy(&viewer);
   }
