...
class Function : public vtkImplicitFunction
{
	...
	void setKoefficents(const float koeff[3])
	{ std::copy(koeff, koeff+3, koeff_); Modified(); }
	
	void getKoefficients(float koeff[3]) const
	{ std::copy(koeff_, koeff_+3, koeff); }

	void setPowers(const float powers[3])
	{ std::copy(powers, powers+3, pow_); Modified(); }
	
	void getPowers(float powers[3]) const
...
class MainWindow : public Gtk::Window
{
	...
	static const float _sc_kx=1, _sc_ky=1, _sc_kz=1;
	static const float _sc_px=2, _sc_py=2, _sc_pz=2;
	static const float _sc_min=0, _sc_max=1;
